<?php // $Id: index.php,v 1.7 2008/03/23 09:11:37 julmis Exp $
 ...
 */
    ...
    $url = $CFG->wwwroot.'/local/cms/index.php?course='.$courseid;
...
/// Get proper context for operations
    ...
    if ( has_capability('local/cms:createmenu', $context, $USER->id) ) {
    	echo '<a href="'.$CFG->wwwroot.'/local/cms/menus.php?course='.$courseid.'&amp;sesskey='.sesskey().'">';
        echo '<img src="'.$OUTPUT->pix_url('menus', 'local_cms').'" width="50" height="50" alt="'.$strmanagemenus.'"
        title="'.$strmanagemenus.'" border="0" /></a><br />';
        echo '<a href="'.$CFG->wwwroot.'/local/cms/menus.php?course='.$courseid.'&amp;sesskey='.sesskey().'">'.$strmanagemenus.'</a>';
    } else {
        echo "&nbsp;";
    }
    ...
    if ( has_capability('local/cms:publishpage', $context, $USER->id) or has_capability('local/cms:createpage', $context, $USER->id) ) {
    	echo '<a href="'.$CFG->wwwroot.'/local/cms/pages.php?course='.$courseid.'&amp;sesskey='.sesskey().'">';
        echo '<img src="'.$OUTPUT->pix_url('pages', 'local_cms').'" width="50" height="50" alt="'.$strmanagepages.'"
        title="'.$strmanagepages.'" /></a><br />';
        echo '<a href="'.$CFG->wwwroot.'/local/cms/pages.php?course='.$courseid.'&amp;sesskey='.sesskey().'">'.$strmanagepages.'</a>';
    } else {
        echo "&nbsp;";
    }
